...
class UpbitChartApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.current_chart_ticker = None # 현재 차트에 그려진 종목 Ticker
    ...
    def load_auto_trade_settings(self):
        try:
            ...
            self.auto_trade_settings = {
                'enabled_tickers': [], 'investment_amount': 5000, 'max_additional_buys': 5,
            }
    ...
    def log_auto_trade(self, message):
        ...
        def update_ui():
            self.log_text.config(state='normal'); self.log_text.insert(END, log_message + "\n")
    ...
    def auto_trade_worker(self):
        ...
        while self.is_auto_trading:
            ...
            except Exception as e: self.log_auto_trade(f"❗️ 자동매매 루프 오류: {e}")
            time.sleep(10)
    ...
    def plot_price_overlays(self):
        ...
        if self.current_price > 0:
            ...
            self.ax.text(1.01, self.current_price, f' {self.current_price:,.2f} ', 
                         transform=blended_transform, color='white', backgroundcolor='red', va='center', ha='left',
        ...
        if ticker:
            ...
            if avg_buy_price > 0:
                ...
                self.ax.text(1.01, avg_buy_price, f' {avg_buy_price:,.2f} ', 
                             transform=blended_transform, color='white', backgroundcolor='blue', va='center', ha='left',
                             bbox=dict(facecolor='blue', alpha=0.8, edgecolor='none', boxstyle='round,pad=0.2'))
...
class AutoTradeSettingsWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        self.vars = {'investment_amount': tk.StringVar(), 'max_additional_buys': tk.StringVar()}
        self.setup_widgets()
        self.load_settings()
    ...
    def setup_widgets(self):
        ...
        self.ticker_listbox.config(yscrollcommand=scrollbar.set); scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        ...
        self.add_buy_combo.pack(side=tk.LEFT); self.add_buy_combo.set('5')
    ...
    def populate_top_tickers(self):
        ...
        except Exception as e: messagebox.showerror("오류", f"종목 목록을 불러오는 중 오류가 발생했습니다:\n{e}", parent=self)
    ...
    def restore_selection(self):
        ...
        for i in range(self.ticker_listbox.size()):
            ...
            if self.master_app.display_name_to_ticker.get(display_name) == selected_ticker:
                self.ticker_listbox.selection_set(i); self.ticker_listbox.activate(i); break

    def save_and_close(self):
        try:
            new_settings = {}; selected_ticker_for_chart = None
            ...
            if selected_indices:
                ...
                if ticker: enabled_tickers.append(ticker); selected_ticker_for_chart = ticker
            new_settings['enabled_tickers'] = enabled_tickers
            amount = int(self.vars['investment_amount'].get())
            if amount < 5000:
                messagebox.showwarning("금액 확인", "최소 주문 금액은 5,000원입니다.", parent=self); return
            ...
            if selected_ticker_for_chart: self.master_app.select_ticker_from_settings(selected_ticker_for_chart)
            messagebox.showinfo("저장 완료", "자동매매 설정이 저장되었습니다.", parent=self)
            self.destroy()
        except ValueError: messagebox.showerror("입력 오류", "매수 금액은 숫자로 입력해야 합니다.", parent=self)
        except Exception as e: messagebox.showerror("오류", f"설정 저장 중 오류가 발생했습니다: {e}", parent=self)
